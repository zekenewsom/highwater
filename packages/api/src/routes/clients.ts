...
router.get('/api/v2/clients', async (req: Request, res: Response) => {
  try {
    ...
    const clients = [
      { id: '1', name: 'Alice Smith', email: 'alice@example.com', advisorId: 'A123', status: 'Active', totalAssets: 2500000, riskProfile: 'Moderate' },
      { id: '2', name: 'Bob Johnson', email: 'bob@example.com', advisorId: 'A456', status: 'Active', totalAssets: 1800000, riskProfile: 'Conservative' },
      { id: '3', name: 'Carol Davis', email: 'carol@example.com', advisorId: 'A123', status: 'Prospect', totalAssets: 0, riskProfile: 'Aggressive' },
    ];
...
router.post('/api/v2/clients/onboard', async (req: Request, res: Response) => {
  try {
    const { name, email, advisorId, riskProfile, initialInvestment } = req.body;
...
router.get('/', async (req: Request, res: Response) => {
  ...
  res.setHeader('X-API-Deprecation-Warning', 'This endpoint is deprecated and will be removed in API v3. Please use /api/v2/clients instead.');
